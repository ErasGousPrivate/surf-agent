...
def list_calendar_events() -> list:
    """
    Fetch existing events on the calendar for the forecast window, so the
    agent can see what a previous run already booked before deciding.
    """
    access_token = _get_google_access_token()
    url = GOOGLE_CALENDAR_EVENTS_URL.format(calendar_id=config.GOOGLE_CALENDAR_ID)
    headers = {"Authorization": f"Bearer {access_token}"}
    time_min = datetime.datetime.utcnow()
    time_max = time_min + datetime.timedelta(days=config.FORECAST_DAYS)
    params = {
...
# ---------------------------------------------------------------------------
# Email reporting -- plain function, not agent-callable (not registered in
# TOOL_FUNCTIONS/TOOL_DEFINITIONS). Called directly by agent.py after the
# agent's tool-use loop finishes.
# ---------------------------------------------------------------------------
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": (
            "Fetch events already on the user's Google Calendar for the "
            "forecast window. Call this FIRST, before deciding anything "
            "else, so you know what a previous run already booked and can "
            "avoid creating a duplicate session for a day/spot that's "
            "already scheduled."
        ),
        "input_schema": {
            "type": "object",
